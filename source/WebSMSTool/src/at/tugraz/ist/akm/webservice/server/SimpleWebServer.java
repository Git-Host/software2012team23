...
public class SimpleWebServer {
		...
		public void run() {
			...
			try {
				...
				httpService.handleRequest(serverConn,
						mWebServer.getHttpContext());
			} catch (Exception e) {
				e.printStackTrace();
				mLog.logE("Exception caught when processing HTTP client connection",
						e);
			}
		}
	}
	...
	private static class ServerThread extends Thread {
		...
		public ServerThread(final SimpleWebServer webServer,
		...
		public void run() {
			...
			while (mRunning) {
				...
				try {
					socket = mServerSocket != null ? mServerSocket.accept()
							: null;
				} catch (IOException e) {
				...
				if (socket != null) {
					mLog.logD("connection request from ip <"
	...
	public synchronized boolean startServer(int port) {
		try {
			if (this.isRunning()) {
				mLog.logI("Web service is already running at port <"
			...
			mLog.logV("Cannot create server socket on port <" + port + ">", e);
			return false;
		}
	}
